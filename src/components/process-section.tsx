...
type IconColor = "emerald" | "blue" | "red" | "amber";

const ICON_STYLES: Record<IconColor, string> = {
  emerald:
    "border-emerald-500/30 bg-gray-100 text-emerald-600 shadow-[0_0_15px_rgba(16,185,129,0.15)] dark:bg-[#151515] dark:text-emerald-500",
  blue: "border-blue-500/30 bg-gray-100 text-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.15)] dark:bg-[#151515] dark:text-blue-400",
  red: "border-red-500/30 bg-gray-100 text-red-500 shadow-[0_0_15px_rgba(239,68,68,0.15)] dark:bg-[#151515]",
  amber:
    "border-amber-500/30 bg-gray-100 text-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.15)] dark:bg-[#151515]",
};

function Icon({ type, color }: { type: keyof typeof ICONS; color: IconColor }) {
  return (
    <div
      className={`z-20 flex h-10 w-10 items-center justify-center rounded-xl border ${ICON_STYLES[color]}`}
    >
      <svg
...
function FlowDiagram() {
  return (
    ...
    >
      <FlowPath
        path="M92 0 V40 C92 60 200 60 200 80 V100"
        gradient="grad_green"
        delay="0s"
      />
      <FlowPath
        path="M164 0 V40 C164 60 200 60 200 80 V100"
        gradient="grad_blue"
        delay="0.5s"
      />
      <FlowPath
        path="M236 0 V40 C236 60 200 60 200 80 V100"
        gradient="grad_red"
        delay="1s"
      />
      <FlowPath
        path="M308 0 V40 C308 60 200 60 200 80 V100"
        gradient="grad_amber"
        delay="1.5s"
      />
  ...
  return (
    <div className="relative z-10 mt-[80px] w-64 rounded-2xl border border-amber-500/50 bg-gray-100 p-4 shadow-[0_0_30px_-10px_rgba(245,158,11,0.3)] dark:bg-[#1E1E1E]">
      ...
      <div className="relative overflow-hidden rounded-lg border border-black/5 bg-white p-3 font-mono text-[10px] leading-relaxed text-gray-500 dark:border-white/5 dark:bg-[#111]">
        ...
        <RecipeProperty name="performance" value="100/100" valueColor="green" />
        <RecipeProperty name="aesthetics" value="Premium" valueColor="green" />
        <RecipeProperty
          name="status"
          value="Ready"
          valueColor="blue"
...
function RecipeProperty({
  ...
  return (
    <p className="font-geist">
      ...
      : <span className={`font-geist ${colorClass}`}>&quot;{value}&quot;</span>
      {hasComma && ","}
    </p>
  );
}
...
function IngredientNode({
  ...
  const dotColors: Record<IconColor, string> = {
  ...
  return (
    ...
    >
      <div
        className={`h-1.5 w-1.5 rounded-full ${dotColors[color]} ${isActive ? "animate-pulse" : ""}`}
      />
        ...
        <div className="relative z-10 grid items-center gap-x-12 gap-y-12 lg:grid-cols-2 lg:gap-24">
          ...
          <div className="pointer-events-none relative order-1 flex w-full flex-col items-center select-none lg:order-2">
            ...
            <div className="relative z-10 mb-8 flex w-full max-w-[400px] justify-center gap-8">
              <Icon type="sprout" color="emerald" />
              <Icon type="fish" color="blue" />
              <Icon type="beef" color="red" />
              <Icon type="flame" color="amber" />
            </div>
